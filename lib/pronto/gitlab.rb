module Pronto
  class Gitlab < Client
    ...
    def pull_id
      env_pull_id || raise(Pronto::Error, "Unable to determine merge request id. Specify either `PRONTO_PULL_REQUEST_ID` or `CI_MERGE_REQUEST_ID`.")
    end

    def env_pull_id
      pull_request = super

      pull_request ||= ENV['CI_MERGE_REQUEST_ID']
